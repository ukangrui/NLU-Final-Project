...
model = MSMRec(user_num = num_u, item_num = num_i, maxlen = 200, num_blocks = 2, num_heads = 1, hidden_units = 50, dropout_rate = 0.2, num_modality=9, meta_emb = torch.zeros((num_i + 1,13824)))

### DEVICE
model.load_state_dict(torch.load('checkpoints/ml-1m-MSMRec-best.pth', map_location=torch.device('cuda')), strict = True)
